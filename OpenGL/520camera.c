...
void camRender(camCamera *cam, GLint viewingLoc) {
	...
	mat444Multiply(proj, isom, projCam);
	GLfloat projectionInverseCamera[4][4];
	
	mat44OpenGL(projCam, projectionInverseCamera);
	glUniformMatrix4fv(viewingLoc, 1, GL_FALSE, (GLfloat *)projectionInverseCamera);
}
