...
void texSample(texTexture *tex, double s, double t) {
	...
	if (tex->filtering == texQUADRATIC) {
		/* My attempt at quadratic filtering */
		
	    double uFrac = u - floor(u);
	    double vFrac = v - floor(v);
	    int currentDim = tex->texelDim;
	    
	    texGetTexel(tex, (int)ceil(u), (int)ceil(v), tex->aux);
	    vecScale(currentDim, uFrac * vFrac, tex->aux, tex->aux);
	    vecCopy(currentDim, tex->aux, tex->sample);
	    
	    texGetTexel(tex, (int)ceil(u), (int)floor(v), tex->aux);
	    vecScale(currentDim, uFrac * (1 - vFrac), tex->aux, tex->aux);
	    vecAdd(currentDim, tex->sample, tex->aux, tex->sample);
	    
	    texGetTexel(tex, (int)floor(u), (int)ceil(v), tex->aux);
	    vecScale(currentDim, (1 - uFrac) * vFrac, tex->aux, tex->aux);
	    vecAdd(currentDim, tex->sample, tex->aux, tex->sample);
	    
	    texGetTexel(tex, (int)floor(u), (int)floor(v), tex->aux);
	    vecScale(currentDim, (1 - uFrac) * (1 - vFrac), tex->aux, tex->aux);
	    vecAdd(currentDim, tex->sample, tex->aux, tex->sample);
	} else
