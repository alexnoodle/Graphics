...
#define renUNIFFOGR 52
#define renUNIFFOGG 53
#define renUNIFFOGB 54
#define renTEXR 0
#define renTEXG 1
#define renTEXB 2
...
pixel. Now it contains all of the implementation for diffuse lighting, specular lighting,
ambient lighting, and fog lighting*/
void colorPixel(renRenderer *ren, double unif[], texTexture (*tex)[], 
    ...
    /* Up until line 112 the diffuse lighting is calculated. The light vector is found
    and compared to the normal vector to find the diffuse intensity */
    ...
    /* Up until line 134 the specular lighting is calculated. The reflection vector and
    the camera vector are found and compared with the light vector in order to find the
    specular intensity */
    double p[3];
    double r[3];
    ...
    /* Application of diffuse lighting and specular lighting that were determined above.
    Ambient lighting is added by adding 0.1 to the diffuse intensity */
    specIntensity = pow(specIntensity, unif[renUNIFSHINY]);
    rgbz[0] = rgbz[0] * (difIntensity + 0.1) * unif[renUNIFLIGHTR] + specIntensity;
    rgbz[1] = rgbz[1] * (difIntensity + 0.1) * unif[renUNIFLIGHTG] + specIntensity;
    rgbz[2] = rgbz[2] * (difIntensity + 0.1) * unif[renUNIFLIGHTB] + specIntensity;
    
    /* the fog calculation */
    rgbz[0] = rgbz[0]*((vary[renVARYZ] + 1.0) / 2.0) + (1.0 - ((vary[renVARYZ] + 1.0)
     / 2.0))*unif[renUNIFFOGR];
    rgbz[1] = rgbz[1]*((vary[renVARYZ] + 1.0) / 2.0) + (1.0 - ((vary[renVARYZ] + 1.0)
     / 2.0))*unif[renUNIFFOGG];
    rgbz[2] = rgbz[2]*((vary[renVARYZ] + 1.0) / 2.0) + (1.0 - ((vary[renVARYZ] + 1.0)
     / 2.0))*unif[renUNIFFOGB];
}
...
double unif[3 + 3 + 3 + 16 + 16 + 1 + 3 + 3 + 3 + 1 + 3];
depthBuffer depth;
...
/* This draws the whole scene and is called by the time step handler. It clears the screen,
    ...
    scene nodes. */
void draw(void){
    pixClearRGB(unif[renUNIFFOGR], unif[renUNIFFOGG], unif[renUNIFFOGB]);
...
int main(void){
    if (pixInitialize(512, 512, "Pixel Stuff") != 0)
        ...
        ren.texNum = 1;
        ren.unifDim = 3 + 3 + 3 + 16 + 16 + 1 + 3 + 3 + 3 + 1 + 3;
        ren.attrDim = 3 + 2 + 3;
        ...
        unif[renUNIFFOGR] = 0.3;
        unif[renUNIFFOGG] = 0.3;
        unif[renUNIFFOGB] = 0.3;
