...
/*** Rendering ***/
/* Needed for storing the vary values */
double varying[renVERTNUMBOUND * renVARYDIMBOUND];

/* Needed to access the correct vary */
...
/* Not necessary for the current spec but it could come in handy later */
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], 
		texTexture (*tex)[]) {
	/* This first transforms all of the vertices and puts them into the global
	vary array and then goes rasterizes one triangle at a time using the appropriate
	vary values  */
	if(mesh->attrDim != ren->attrDim){
	    ...
	    for(int i = 0; i < mesh->triNum; i += 1){
	        ...
	        clipRender(ren, unif, tex, varyA, varyB, varyC);
	    }
	}
	return;
}
