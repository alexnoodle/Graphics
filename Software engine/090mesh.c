...
double varying[renVERTNUMBOUND * renVARYDIMBOUND];

double *meshGetVaryPointer(meshMesh *mesh, int vary, renRenderer *ren) {
	if (0 <= vary && vary < mesh->vertNum)
		return &varying[vary * ren->varyDim];
	else
		return NULL;
}

void meshSetVary(meshMesh *mesh, int vary, double attr[]) {
	int k;
	if (0 <= vary && vary < mesh->vertNum)
		for (k = 0; k < mesh->attrDim; k += 1)
			varying[mesh->attrDim * vary + k] = attr[k];
}
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], 
	...
	if(mesh->attrDim != ren->attrDim){
	    printf("Error: Attribute values do not match up\n");
	}
	else{
	    for(int i = 0; i < mesh->vertNum; i += 1){
	        double *vert = meshGetVertexPointer(mesh, i);
	        double *vary = meshGetVaryPointer(mesh, i, ren);
	        ren->transformVertex(ren, unif, vert, vary);
	    }
	    for(int i = 0; i < mesh->triNum; i += 1){
	        printf("inside the bulk of meshRender\n");
	        int *tri = meshGetTrianglePointer(mesh, i);
	        
	        double *varyA = meshGetVaryPointer(mesh, tri[0], ren);    
	        double a[4] = {varyA[0], varyA[1], varyA[2], varyA[3]};
	        //printf("vertex A: %f, %f, %f, %f\n", varyA[0], varyA[1], varyA[2], varyA[3]);
	          
	        double *varyB = meshGetVaryPointer(mesh, tri[1], ren);
	        double b[4] = {varyB[0], varyB[1], varyB[2], varyB[3]};
	        //printf("vertex B: %f, %f, %f, %f\n", varyB[0], varyB[1], varyB[2], varyB[3]);
	          
	        double *varyC = meshGetVaryPointer(mesh, tri[2], ren);
	        double c[4] = {varyC[0], varyC[1], varyC[2], varyC[3]};
	        //printf("vertex C: %f, %f, %f, %f\n", varyC[0], varyC[1], varyC[2], varyC[3]);
	            
	        printf("we made it to triRender Once\n");
...
int meshInitializeRectangle(meshMesh *mesh, double left, double right, 
	...
	if (error == 0) {
		...
		double attr[4];
		vecSet(attr, 4, left, bottom, 0.0, 0.0);
		meshSetVertex(mesh, 0, attr);
		vecSet(attr, 4, right, bottom, 1.0, 0.0);
		meshSetVertex(mesh, 1, attr);
		vecSet(attr, 4, right, top, 1.0, 1.0);
		meshSetVertex(mesh, 2, attr);
		vecSet(attr, 4, left, top, 0.0, 1.0);
...
int meshInitializeEllipse(meshMesh *mesh, double x, double y, double rx, 
	...
	if (error == 0) {
		...
		for (i = 0; i < sideNum; i += 1) {
			...
			cosTheta = cos(theta);
			sinTheta = sin(theta);
			vecSet(attr, 4, x + rx * cosTheta, y + ry * sinTheta, 
